...
class ProductController extends PublicController
{
    ...
    public function lists()
    {
        ...
        //*****
        if (intval($id)) {
            $catList = M('category')->where('id=' . intval($id))->getField('tid');
            if (intval($catList) > 1) {
                $map['cid'] = intval($id);
            } else {
                $childlist = M('category')->where('tid=' . intval($id))->getField('id', true);
                $map['cid'] = array('in', $childlist);
            }
        }
        //*****
        ...
        $product = M('product')->where($map)->where($where)->order($order)->limit($page . ',20')->select();
