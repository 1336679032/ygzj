...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 产品管理 <span
        class="c-gray en">&gt;</span> 全部产品 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
        ...
        <?php if(is_array($productlist)): $i = 0; $__LIST__ = $productlist;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td>
                    <p id="new_<?php echo ($v["id"]); ?>">
                        <?php if($v["is_show"] == 1): ?><a class="label blue" onclick="pro_new(<?php echo ($v["id"]); ?>,1);">新品上市
                            <?php else: ?>
                            <a class="label err" onclick="pro_new(<?php echo ($v["id"]); ?>,0);">非新品<?php endif; ?>
                        </a></p>
                    <p id="hot_<?php echo ($v["id"]); ?>" style="margin-top:5px;">
                        <?php if($v["is_hot"] == 1): ?><a class="label succ" onclick="pro_hot(<?php echo ($v["id"]); ?>,1);">热卖商品
                            <?php else: ?>
                            <a class="label err" onclick="pro_hot(<?php echo ($v["id"]); ?>,0);">非热卖<?php endif; ?>
                        </a></p>
                ...
                <td>
                    <!-- <?php if($v["pro_buff"] != ''): ?><a href="<?php echo U('Product/proGuige');?>?pid=<?php echo ($v["id"]); ?>">
                    <?php else: ?>
                    <a href="<?php echo U('Product/setAttr');?>?pid=<?php echo ($v["id"]); ?>"><?php endif; ?><font style="color:red;">属性设置</font></a> | -->
                    <a href="<?php echo U('setTJ');?>&pro_id=<?php echo ($v["id"]); ?>&page=<?php echo ($page); ?>&name=<?php echo ($name); ?>&shop_id=<?php echo ($shop_id); ?>&tuijian=<?php echo ($tuijian); ?>">推荐</a>
                    |
...
<script>
    ...
    function pro_new(pro_id, type) {
        ...
        $.post("<?php echo U('Product/setNew');?>", {pro_id: pro_id}, function (data) {
            if (data.status == 1) {
                if (type == 1) {
    ...
    function pro_hot(pro_id, type) {
        ...
        $.post("<?php echo U('Product/setHot');?>", {pro_id: pro_id}, function (data) {
            if (data.status == 1) {
                if (type == 1) {
    ...
    function pro_zk(pro_id, type) {
        ...
        $.post("<?php echo U('Product/setZK');?>", {pro_id: pro_id}, function (data) {
            if (data.status == 1) {
                if (type == 1) {
