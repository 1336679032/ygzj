...
class ProductController extends PublicController
{
    ...
    public function index()
    {
        ...
        //搜索变量
        $type = $this->htmlentitiesU8($_GET['type']);
        $tuijian = $this->htmlentitiesU8($_GET['tuijian']);
        $name = $this->htmlentitiesU8($_GET['name']);
        //===============================
        // 产品列表信息 搜索
        //===============================
        //搜索
        ...
        $page < 0 ? $page = 0 : '';
        $limit = $page * rows;
        $page_index = $this->pageIndex($count, $rows, $page);
        ...
        if ($_POST['submit'] == true) {
            try {
                ...
                if (!empty($_FILES["photo_x"]["tmp_name"])) {
                    //文件上传
                    $info = $this->uploadImages($_FILES["photo_x"], array('jpg', 'png', 'jpeg'), "product/" . date(Ymd));
                ...
                if (!empty($_FILES["photo_d"]["tmp_name"])) {
                    //文件上传
                    $info = $this->uploadImages($_FILES["photo_d"], array('jpg', 'png', 'jpeg'), "product/" . date(Ymd));
                    if (!is_array($info)) {// 上传错误提示错误信息
                        $this->error($info);
                        exit();
                    } else {// 上传成功 获取上传文件信息
                ...
                if ($up_arr) {
                    ...
                    foreach ($up_arr as $key => $value) {
                        $res = $this->uploadImages($value, array('jpg', 'png', 'jpeg'), "product/" . date(Ymd));
                        if (is_array($res)) {
                            // 上传成功 获取上传文件信息保存数据库
    ...
    */
    ...
    public function saveGuide()
    {
        ...
        if ($_POST['attribute']) {
            ...
            foreach ($attribute as $k => $v) {
                ...
                if ($guige_name) {
                    foreach ($guige_name as $key => $val) {
                        ...
                        if (!$res) {
                            $this->error('部分规格添加失败，请核对过后再补充！', U('pro-guige', array('pid' => $pro_id)));
                            exit();
                        }
                    }
                }
            ...
            $this->success('操作成功！', U('pro-guige', array('pid' => $pro_id)));
            exit();
        }
    ...
    public function guigeUpload()
    {
        ...
        if (!empty($_FILES['file_' . $id]['tmp_name'])) {
            //文件上传
            $info = $this->uploadImages($_FILES['file_' . $id], array('jpg', 'png', 'jpeg'), "attribute/" . date(Ymd));
            if (!is_array($info)) {// 上传错误提示错误信息
                $this->error($info);
                exit();
            } else {// 上传成功 获取上传文件信息
        ...
        $this->redirect('pro-guige', array('pid' => intval($check_info['pid'])));
    }
    ...
    //**********************************************
    public function setProAttr($pro_id)
    {
        //查询产品是否存在
