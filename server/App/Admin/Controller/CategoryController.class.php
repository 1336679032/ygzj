...
class CategoryController extends PublicController
{
    ...
    */
    public function save()
    {
        ...
        if (!empty($_FILES["file2"]["tmp_name"])) {
            //文件上传
            $info2 = $this->uploadPic($_FILES["file2"], array('jpg', 'png', 'jpeg'), "category/" . date(Ymd));
        ...
        if (!empty($_FILES["file3"]["tmp_name"])) {
            //文件上传
            $info3 = $this->uploadPic($_FILES["file3"], array('jpg', 'png', 'jpeg'), "category/" . date(Ymd));
    ...
    */
    private function uploadPic($file, $exts, $path)
    {
