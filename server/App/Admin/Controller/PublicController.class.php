...
//****************************************
    ...
    //原作者自己封装的分页功能
    //js写法，直接复制。
    //function productOption(page){
    //  window.location.href='?page='+page+'&message='+$("#message").val()
    //}
    //**************************
    public function pageIndex($count, $row_page, $page)
    {
        $pare = $page > 0 ? '<a onclick="productOption(' . ($page - 1) . ');">上一页</a>' : '<span>上一页</span>';
        $next = $row_page - 1 > $page ? '<a onclick="productOption(' . ($page + 1) . ');">下一页</a>' : '<span>下一页</span>';
        ...
        $text = '<span style="color:#666">
			   ...
			   ' . $pare . $next . '&nbsp;&nbsp; 
			   <select onchange="productOption(this.value)">';
    ...
    //中文字符串截取
    //****************
    public function strSubstr($title, $length)
    {
        $encoding = 'utf-8';
    ...
    //方法名称：htmlentitiesU8。
    //*********************************
    public function htmlentitiesU8($v)
    {
    ...
    //方法名称：htmlDecodeU8。
    //************************************
    public function htmlDecodeU8($v)
    {
    ...
    */
    public function uploadImages($file, $exts, $path)
    {
