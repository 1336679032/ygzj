...
class MoreController extends PublicController
{
    ...
    public function fankui()
    {
        ...
        if (!empty($_GET)) {
            //dump(I('get.'));exit;
            $message = $this->htmlentitiesU8($_GET['message']);
            if ($_GET['type'] == 'del') {
    ...
    public function saveimg()
    {
        ...
        if (!empty($_FILES["file"]["tmp_name"])) {
            //文件上传
            $info = $this->uploadImages($_FILES["file"], array('jpg', 'png', 'jpeg'), "category/indeximg");
    ...
    public function city()
    {
        ...
        //dump($city);exit;
        //省市区面包屑，此调用函数在楼下。
        $nav = $id > 0 ? $this->cityJibie($id) : NULL;
        //dump($_GET);
    ...
    public function cityAdd()
    {
        ...
        if ($_POST['submit']) {
            ...
            $array = array(
                'tid' => $tid,
                'name' => $this->htmlentitiesU8($_POST['name']),
            );
            //此处为添加下级
            if ($id < 1) {
    ...
    public function setup()
    {
        if (IS_POST) {
            ...
            if (!empty($_FILES["file2"]["tmp_name"])) {
                //文件上传
                $info2 = $this->uploadImages($_FILES["file2"], array('jpg', 'png', 'jpeg'), "logo");
